...
const verifiedRoleRemoveRole = new SlashCommandSubcommand(
  "remove",
  "Remove a role",
  undefined,
...
verifiedRoleRemoveRole.setExecuteFunction(async (command) => {
  try {
    ...
    collector.on("collect", async (menu: StringSelectMenuInteraction) => {
      try {
        ...
        for (const role of roles) {
          await verifiedroles.deleteOne({ _id: role });
        }
...
export default verifiedRoleRemoveRole;
